...
if ($_SERVER["REQUEST_METHOD"] == "POST") {  
    ...
    if ($stmtCheck->num_rows > 0) {  
        ...
        if ($estadoCuenta === "Bloqueado") {  
            $message = "<div class='bg-red-100 text-red-800 p-4 rounded mb-4'>La cuenta está bloqueada. No puede realizar depósitos.</div>";  
        } else {  
            ...
            if ($stmtUpdate->execute()) {  
                ...
                // Verificar si ya existe un registro de transacción para esta cuenta y fecha  
                $sqlCheckTransaction = "SELECT * FROM transacciones WHERE fecha = ? AND tipo = ?";  
                $stmtCheckTransaction = $conn->prepare($sqlCheckTransaction);  
                $stmtCheckTransaction->bind_param("ss", $fecha, $tipo);  
                $stmtCheckTransaction->execute();  
                $stmtCheckTransaction->store_result();  

                if ($stmtCheckTransaction->num_rows == 0) {  
                    $sqlInsert = "INSERT INTO transacciones (fecha, tipo) VALUES (?, ?)";  
                    $stmtInsert = $conn->prepare($sqlInsert);  
                    $stmtInsert->bind_param("ss", $fecha, $tipo);  
                    
                    if ($stmtInsert->execute()) {  
                        $message = "<div class='bg-green-100 text-green-800 p-4 rounded mb-4'>Depósito realizado con éxito. Saldo actual: $$nuevoSaldo.</div>";  
                    } else {  
                        $message = "<div class='bg-red-100 text-red-800 p-4 rounded mb-4'>Error al registrar la transacción: " . $stmtInsert->error . "</div>";  
                    }  
                    $stmtInsert->close();  
                } else {  
                    $message = "<div class='bg-yellow-100 text-yellow-800 p-4 rounded mb-4'>La transacción ya ha sido registrada.</div>";  
                }  
...
<head>  
    ...
    <title>Depósito Monetario - Banca Electrónica</title>  
                ...
                xhr.onload = function() {  
                    if (xhr.status === 200) {  
                        ...
                        if (response.includes('Cuenta no encontrada')) {  
                            ...
                            document.getElementById('accountMessage').textContent = 'La cuenta está bloqueada. No puede realizar depósitos.';  
                            document.getElementById('accountMessage').classList.remove('hidden');  
                        } else {  
...
<body class="bg-gray-100 flex flex-col min-h-screen">  
    ...
    <main class="container mx-auto mt-8 flex-grow flex flex-col items-center">  
        <section class="bg-white p-6 rounded-lg shadow-md w-full max-w-3xl">  
            <h2 class="text-2xl font-semibold mb-4 text-center">Depósito Monetario</h2>  
            <form id="depositoForm" method="POST" action="" onsubmit="disableSubmitButton()">  
                ...
                <div class="mb-4">  
                    <label for="cantidadDeposito" class="block text-sm font-medium text-gray-700">Cantidad a Depositar</label>  
                    <input type="number" id="cantidadDeposito" name="cantidadDeposito" required class="mt-1 block w-full border border-gray-300 rounded-md p-2" placeholder="Cantidad a depositar" min="0" step="0.01">  
                </div>  
                <button type="submit" class="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 rounded">Depositar</button>  
            </form>  
