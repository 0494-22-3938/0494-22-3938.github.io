...
<main class="container mx-auto mt-8 flex-grow flex items-center justify-center">  
    <section class="bg-white p-6 rounded-lg shadow-md w-full max-w-sm">  
        <h2 class="text-xl font-semibold mb-4 text-center">Iniciar Sesión - Cajero</h2>  
        <?php  
        session_start();  
        $error_message = '';  
        ...
        if ($_SERVER["REQUEST_METHOD"] == "POST") {  
            ...
            $usuario = $_POST['cashier-username'];  
            $contrasena = $_POST['cashier-password'];  
            ...
            if ($result->num_rows === 1) {  
                ...
                if ($row['rol'] === 'Clien') {  
                    // Redirigir a la página de inicio del cajero  
                    header("Location: HOME_CAJA.html");  
                    exit();  
                } else {  
        ...
        <?php endif; ?>  
        <form action="LOGONC.php" method="POST">  
            <div class="mb-4">  
                <label for="cashier-username" class="block text-gray-700">Nombre de Usuario</label>  
                <input type="text" id="cashier-username" name="cashier-username" class="mt-1 block w-full p-2 border border-gray-300 rounded" required />  
            </div>  
            <div class="mb-4">  
                <label for="cashier-password" class="block text-gray-700">Contraseña</label>  
                <input type="password" id="cashier-password" name="cashier-password" class="mt-1 block w-full p-2 border border-gray-300 rounded" required />  
            </div>  
